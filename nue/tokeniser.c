...
static bool isAtEnd(const Tokeniser* lexer) {
    return *lexer->current == '\0';
}

// advance the lexer pointer to the next character and return the current character
static char advance(Tokeniser* lexer) {
    lexer->current++;
    return lexer->current[-1];
}

// peek at the current character without consuming it
static char peek(const Tokeniser* lexer) {
    return *lexer->current;
}

// peek at the next character without consuming it
static char peekNext(const Tokeniser* lexer) {
    if (isAtEnd(lexer)) return '\0';
    return lexer->current[1];
}

// skip whitespace and comments
static void skipWhitespace(Tokeniser* lexer) {
    // for non-c programmers, this is an infinite loop.
    // (in our case, it's not entirely infinite because we return at some point anyway)
    // you have to love the C bible
    while (1) {
...
static bool match(Tokeniser* lexer, const char expected) {
...
// create a token
static Token makeToken(const Tokeniser* lexer, const TokenType type, void *literal) {
    Token token;
    token.type = type;
...
static Token errorToken(const Tokeniser* lexer, const char* message) {
    Token token;
...
static Token number(Tokeniser* lexer) {
    // this function became so unmaintainable that I have decided to actually comment it for better uh, comprehension?
    bool hasDot = false;
    bool isHex = false;
    bool isBinary = false;
    bool isOctal = false;
    ...
    if (*numberStart == '0') {
        const char nextChar = peek(lexer);
        ...
        // there is no need to advance if its leading zero in a decimal number
    }
    ...
    const size_t lexemeLength = lexemeEnd - numberStart;
    ...
    if (isHex) {
        ...
        value = 0;
        const size_t startIndex = 2; // skip "0b"
...
// CLEAN THIS CODE UP
static Token string(Tokeniser* lexer, const char quote) {
    ...
    while (!isAtEnd(lexer)) {
        ...
        if (c == quote) {
            ...
            if (matchedQuotes == quoteCount) {
                ...
                buffer[bufferIndex] = '\0'; // null terminates the string
            ...
            const char escapeChar = advance(lexer);
            if (escapeChar == 'x') {
                ...
                while (isxdigit(peek(lexer))) {
                    const char hexDigit = advance(lexer);
                ...
                const int unicodeLength = (escapeChar == 'u') ? 4 : 8;
                int unicodeValue = 0;
    ...
    // obviously at this point, if all the 'if' statements didn't catch every character in a string, or it just wasn't terminated, then return error
    free(buffer);
    return errorToken(lexer, "Unterminated string");
}

// scan identifier or keyword
static Token identifier(Tokeniser* lexer) {
    // you might question whether the identifier function allows identifiers to start with a number...
    // however, that's not the case because in the scanToken we initially check with isalpha() which doesnt allow numbers
    ...
    if (length == 6 && strncmp(lexer->start, "repeat", 6) == 0) return makeToken(lexer, TOKEN_REPEAT, NULL);
    if (length == 5 && strncmp(lexer->start, "until", 5) == 0) return makeToken(lexer, TOKEN_UNTIL, NULL);
    ...
    if (length == 2 && strncmp(lexer->start, "as", 2) == 0) return makeToken(lexer, TOKEN_AS, NULL);
...
Token scanToken(Tokeniser* lexer) {
    ...
    switch (c) {
        ...
        case '.':
            if (match(lexer, '.')) return makeToken(lexer, match(lexer, '.') ? TOKEN_ELLIPSIS : TOKEN_DOT_DOT, NULL);
